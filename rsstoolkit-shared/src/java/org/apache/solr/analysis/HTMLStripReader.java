...
public class HTMLStripReader extends Reader {
  ...
  /**
   * @see #setKeepAlignedPositions(boolean)
   */
  private boolean keepAlignedPositions = true;
  ...
  public static void main(String[] args) throws IOException {
    Writer w = new OutputStreamWriter(new FileOutputStream("out2.log"), "UTF-8");
    for (Map.Entry<String,Character> e : entityTable.entrySet())
    {
        w.write(e.getKey() + " -> " + e.getValue()
            + " &" + e.getKey() + ";  &" + e.getKey().toUpperCase() + ";");
        w.write("<br/>\n");
    }
    w.close();
    System.exit(0);
...
*/
  ...
  private int readNumericEntity() throws IOException {
    ...
    // the entity.
    try {
      if (base == 16) numWhitespace = 1; // + 1 accounts for 'x' in hexadecimal entities. 
      if (ch==';' || ch==-1) {
      ...
      if (isSpace(ch)) {
        push(ch);
        numWhitespace += sb.length() + 1;// + 1 accounts for &, # then, take away 1 for the fact that we do output a char
        return Integer.parseInt(sb.toString(), base);
      }
  ...
  private int readEntity() throws IOException {
     ...
     * 
     * This seems consistent with what firefox does, it requires
     * at least a white space after an entity. 
     */
  ...
  ***/
  ...
  public int read() throws IOException {
    ...
    if (keepAlignedPositions && numWhitespace > 0) {
      numWhitespace--;
      return ' ';
    }
  ...
  /**
   * If <code>true</code>, the reader keeps the input stream and 
   * output stream of characters position-aligned (the same 
   * characters have corresponding offsets in both streams). Entities and
   * HTML blocks are padded with white space. If <code>false</code>, 
   * HTML blocks are removed from the stream and entities are replaced with
   * their unicode equivalents.
   */
  public void setKeepAlignedPositions(boolean keepAligned) {
  ...
  static {
    ...
    // Exceptions that can also be written in uppercase (FF, IE).
    final String [] uppercaseExceptions = { "quot", "copy", "gt", "lt", "reg", "amp" };
    for (int i=0; i<uppercaseExceptions.length; i++) {
        entityTable.put(uppercaseExceptions[i].toUpperCase(), 
            entityTable.get(uppercaseExceptions[i]));
    }
