...
public class HTMLStripReaderTest extends TestCase {
  ...
  public void testNoPadding() throws Exception {
    // With padding
    String test    = ":&oacute;:&amp;:&lt;:&#64;:&#x20;:&#x20 :";
    String with    = ":ó       :&    :<   :@    :      :      :";
    String without = ":ó:&:<:@: :  :";

    assertEquals(with, read(test));
    
    HTMLStripReader reader = new HTMLStripReader(new StringReader(test));
    reader.setKeepAlignedPositions(false);
    assertEquals(without, read(reader));
  }

  public void testUppercaseExceptions() throws Exception {
      // With padding
      String test    = ":&OACUTE;:&AMP;:&LT;:&COPY;:";
      String with    = ":&OACUTE;:&    :<   :©     :";
  ...
  private String read(String input) throws IOException {
    return read(new HTMLStripReader(new StringReader(input)));
  }
}